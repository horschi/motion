...
void alg_noise_tune(struct context *cnt, unsigned char *new)
{
    ...
    int maskcorrection = cnt->conf.mask_changedpixel_correction_percent;
    ...
    for (; i > 0; i--) {
        ...
        {
            if(maskcorrection > 0)
                diff += ((diff * maskcorrection * (255-*mask++)) / (255*100));
            else
                diff = ((diff * *mask++) / 255);
        }
...
int alg_diff_standard(struct context *cnt, unsigned char *new)
{
    ...
    int maskdiffs = 0;
    int maskcorrection = cnt->conf.mask_changedpixel_correction_percent;
    int masknoise = (noise * cnt->conf.mask_noise_correction_percent) / 100;
    int x,y,xd,yd;
    for (y=0; y < height; y++)
    {
        for (x=0; x < width; x++)
        {
            ...
            if (mask)
            {
                ...
                if(maskcorrection > 0 && correcteddiff < masknoise && curdiff > masknoise)
                {
                    maskdiffs++;
                }
